...
type NoticeAppeal = Database["public"]["Tables"]["notice_appeals"]["Row"];
...
const noticeSchema = z.object({
  ...
  evidence: z
    .object({
      ...
      private_object_paths: z.array(z.string().trim().min(3).max(500)).max(8).default([]),
      sha256: z
        .string()
        .regex(/^[a-f0-9]{64}$/)
        .optional(),
...
export const issueParkingNoticeFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: notice, error } = await context.supabase.rpc("issue_parking_notice_v2", {
      _site_id: data.site_id,
      _plate: data.plate,
      _reason: data.reason,
...
export const createNoticeAppealFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: unknown) =>
    z
      .object({
  ...
  .handler(async ({ data, context }) => {
    const { data: appeal, error } = await context.supabase.rpc("create_notice_appeal", {
      _notice_id: data.notice_id,
      _reason: data.reason,
      _details: data.details,
    });
    rpcError(error);
    return appeal as NoticeAppeal;
  });

export const resolveNoticeAppealFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: unknown) =>
    z
      .object({
        appeal_id: z.string().uuid(),
        decision: z.enum(["accepted", "upheld"]),
        response: z.string().trim().min(10).max(4000),
      })
      .parse(input),
  )
  .handler(async ({ data, context }) => {
    const { data: appeal, error } = await context.supabase.rpc("resolve_notice_appeal", {
      _appeal_id: data.appeal_id,
...
export const createNoticePaymentFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: unknown) => z.object({ notice_id: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    const { data: payment, error } = await context.supabase.rpc("create_notice_payment", {
      _notice_id: data.notice_id,
    });
    rpcError(error);
...
export const markNotificationReadFn = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input: unknown) => z.object({ notification_id: z.string().uuid() }).parse(input))
  .handler(async ({ data, context }) => {
    const { data: notification, error } = await context.supabase.rpc("mark_notification_read", {
