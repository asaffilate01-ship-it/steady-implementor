  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const operatorOrgIds = new Map<string, string>();
        const operatorOrgId = async (name: string) => {
          const cached = operatorOrgIds.get(name);
          ...
          const { data: existing } = await supabaseAdmin
            ...
            .maybeSingle();
          if (existing?.id) {
            operatorOrgIds.set(name, existing.id);
            return existing.id;
          }
          ...
          operatorOrgIds.set(name, created.id);
          return created.id;
        };
        ...
        for (const prov of providers ?? []) {
          ...
          try {
            ...
            for (const u of upstream) {
              const orgId = await operatorOrgId(u.operator_name);
              const normalizedType: "street" | "garage" | "lot" =
                u.type === "on-street" || u.type === "street"
                  ? "street"
                  : u.type === "lot"
                    ? "lot"
                    : "garage";
              ...
              if (existing?.site_id) {
                const { error: siteError } = await supabaseAdmin
                  ...
                  .update({
                    ...
                    price_cents_per_hour: u.price_cents_per_hour,
                    type: normalizedType,
                ...
                const { data: ns, error: siteError } = await supabaseAdmin
                  .from("sites")
                  .insert({
                    org_id: orgId,
                    name: u.name,
                  ...
                  .select()
                  .single();
                if (siteError || !ns) throw new Error(siteError?.message ?? "Site insert failed");
                const { error: mappingError } = await supabaseAdmin
                  .from("site_provider_mapping")
                  .insert({
                    site_id: ns.id,
                    provider_id: prov.id,
                    external_site_id: u.external_id,
