...
function EnforcementApp() {
  ...
  const [queuedDraft, setQueuedDraft] = useState(false);
  ...
  useEffect(() => {
    ...
    setQueuedDraft(!!localStorage.getItem("parkpunkt.enforcement-draft"));
    return () => {
  ...
  async function uploadEvidence() {
    if (!evidenceFile) return { path: null, sha256: null };
    ...
    const sha256 = Array.from(new Uint8Array(digest))
      ...
      .join("");
    const safeName = evidenceFile.name.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(-100);
    const path = `${userId}/${Date.now()}-${crypto.randomUUID()}-${safeName}`;
    const { error } = await supabase.storage.from("notice-evidence").upload(path, evidenceFile, {
  ...
  function saveOfflineDraft() {
    ...
    localStorage.setItem("parkpunkt.enforcement-draft", JSON.stringify(draft));
    setQueuedDraft(true);
    toast.success("Notice draft stored securely on this device until connectivity returns");
  }

  function restoreOfflineDraft() {
    const raw = localStorage.getItem("parkpunkt.enforcement-draft");
    if (!raw) return;
    try {
      ...
      if (draft.plate) setPlate(draft.plate);
      if (draft.reason) setReason(draft.reason);
      if (typeof draft.amount === "number") setAmount(draft.amount);
      if (draft.officerNote) setOfficerNote(draft.officerNote);
      toast.success("Offline draft restored. Reattach its evidence file before issuing.");
    } catch {
      localStorage.removeItem("parkpunkt.enforcement-draft");
      ...
      {queuedDraft && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm">
          <span className="flex items-center gap-2">
            ...
            An offline notice draft is waiting on this device.
          </span>
          <Button size="sm" variant="outline" onClick={restoreOfflineDraft}>
            Restore draft
          </Button>
        </div>
      )}
      ...
      <div className="grid gap-6 md:grid-cols-2">
          ...
          <CardContent className="space-y-3">
            ...
            <Field label={t("enf.plate")}>
              <div className="flex gap-2">
                ...
                />
                <Button disabled={scanning || !siteId || !isOnline} onClick={runScan}>
            ...
            {!isOnline && (
              <p className="rounded-lg bg-secondary p-2 text-xs text-muted-foreground">
                Live entitlement checks pause offline to prevent false notices. You can retain
                observations as a draft and verify after reconnection.
              </p>
            )}
            {scan &&
              (scan.status === "valid" ? (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <Field label="Private evidence upload (optional)">
              <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border p-3 text-sm transition hover:bg-secondary/50">
                <span className="grid h-9 w-9 place-items-center rounded-lg bg-primary/10 text-primary">
                  <CloudUpload className="h-4 w-4" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-medium">
                    {evidenceFile?.name ?? "Photo or PDF, up to 10 MB"}
                  </span>
                  <span className="block text-xs text-muted-foreground">
                    Private bucket · SHA-256 evidence fingerprint
                  </span>
                </span>
                <Input
                  className="sr-only"
            ...
            <Field label="Existing secure evidence URL (optional)">
              <Input
                type="url"
                value={photoUrl}
                ...
                try {
                  ...
                  await issue.mutateAsync({
                    ...
                    evidence: {
                      ...
                      private_object_paths: uploaded.path ? [uploaded.path] : [],
                      sha256: uploaded.sha256 ?? undefined,
                      content_type: evidenceFile?.type ?? undefined,
                      original_filename: evidenceFile?.name ?? undefined,
                    },
                  });
            ...
            >
              {isOnline ? t("enf.issue") : "Save notice draft offline"}
            </Button>
            {scan?.status === "valid" && (
